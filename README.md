...
4. More games
 ...
 - 4.3 Listening game, similar to game from 4.1 but this time computer reads words from useWordSetup composable and player must select correct letters in correct order. There should be a progress bar displayed and for every correct guess player advances on progress bar by 1. For every incorrect guess the player goes back on progress bar by one. The progress bar maximum value is 10. The game reuses existing composables.
